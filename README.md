...
This project is a real-time IoT Weather Monitoring System that collects environmental data and determines its location without a GPS module. It utilizes an architecture of IoT + Edge Computing + Cloud to handle data processing and storage efficiently.
...
* **Sensing Layer**: ESP8266/ESP32 collects DHT11 data and scans nearby Wi-Fi SSIDs.
...
2. **Geolocation**: Instead of a physical GPS module, the device sends nearby Wi-Fi MAC addresses to the Google Geolocation API to receive Latitude and Longitude.
...
* ESP8266 / ESP32 (Microcontroller with integrated Wi-Fi)
...
```
├── main.cpp            # Main Arduino sketch for ESP8266/ESP32
├── geolocation.h        # Logic for Google API communication
├── nearest_city.h     # Edge module for city mapping (Haversine)
...
* **Serial Monitor**: Displays real-time logs including Latitude/Longitude, Temperature/Humidity, and the identified Nearest City.
* **ThingSpeak**: Visual live charts showing environmental trends over time.
* **AWS RDS**: A MySQL database table populated with location-tagged weather records for long-term storage.

## Achievement

1st Prize Winner at the IoT & Edge Computing Hackathon. This project was recognized for overcoming the memory limitations of microcontrollers by offloading heavy geospatial calculations to the Edge, and for providing accurate location data without the need for expensive or indoor-limited GPS hardware.
